...
export const uploadDocument = async (orgId, uploadedBy, file, filePath) => {
    const membership = await pool.query(`SELECT 1 FROM organization_members
     WHERE organization_id = $1 AND user_id = $2`, [orgId, uploadedBy]);
    if (membership.rowCount === 0) {
        throw new Error("NOT_ORG_MEMBER");
    }
    const docId = crypto.randomUUID();
    const result = await pool.query(`INSERT INTO documents (
      id,
      organization_id,
      uploaded_by,
      file_name,
...
export const getAllDocuments = async (orgId, userId) => {
...
export const getDocumentById = async (docId, orgId, userId) => {
    // Check if user is a member of the organization
    const membership = await pool.query(`SELECT 1 FROM organization_members
     WHERE organization_id = $1 AND user_id = $2`, [orgId, userId]);
    if (membership.rowCount === 0) {
    ...
    const result = await pool.query(`SELECT d.*, 
     ...
     FROM documents d
     LEFT JOIN users u ON d.uploaded_by = u.id
     WHERE d.id = $1 AND d.organization_id = $2`, [docId, orgId]);
...
export const updateDocument = async (docId, orgId, userId, updates) => {
    // Check if document exists
    const docCheck = await pool.query(`SELECT uploaded_by FROM documents
    ...
    const adminCheck = await pool.query(`SELECT role FROM organization_members
     WHERE organization_id = $1 AND user_id = $2`, [orgId, userId]);
...
export const deleteDocument = async (docId, orgId, userId) => {
    // Check if document exists
    const docCheck = await pool.query(`SELECT uploaded_by, file_path FROM documents
     WHERE id = $1 AND organization_id = $2`, [docId, orgId]);
    if (docCheck.rowCount === 0) {
        throw new Error("DOCUMENT_NOT_FOUND");
    }
