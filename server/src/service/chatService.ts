...
export async function createConversation(
    ...
    const result = await pool.query(
        ...
        [orgId, userId, title || 'New Conversation']
    );
...
export async function getAdvancedAnalytics(
    ...
    const frequentQuestionsResult = await pool.query(
        `SELECT 
    ...
    const documentUsageResult = await pool.query(
        `SELECT 
            d.id,
            d.file_name,
    ...
    const activityTimelineResult = await pool.query(
        `SELECT 
