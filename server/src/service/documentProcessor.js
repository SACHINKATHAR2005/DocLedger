...
export async function processDocument(documentId) {
    try {
        ...
        const documentResult = await pool.query(`SELECT id, organization_id, file_path, file_name, file_type, status 
       FROM documents 
       ...
       WHERE id = $2`, [DocumentStatus.PROCESSING, documentId]);
        ...
        for (let i = 0; i < chunks.length; i++) {
            const chunkResult = await pool.query(`INSERT INTO document_chunks 
         (document_id, chunk_index, chunk_text, organization_id, created_at)
         VALUES ($1, $2, $3, $4, NOW())
         RETURNING id`, [documentId, i, chunks[i], document.organization_id]);
        ...
        for (let i = 0; i < chunks.length; i += BATCH_SIZE) {
            ...
            console.log(`[Worker] Generated embeddings for batch ${Math.floor(i / BATCH_SIZE) + 1}/${Math.ceil(chunks.length / BATCH_SIZE)}`);
        ...
        for (let i = 0; i < allEmbeddings.length; i++) {
            ...
            await pool.query(`INSERT INTO document_embeddings 
        ...
        await pool.query(`UPDATE documents 
       SET status = $1, updated_at = NOW() 
       WHERE id = $2`, [DocumentStatus.READY, documentId]);
