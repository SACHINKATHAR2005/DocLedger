...
export async function insertDocumentEmbeddings(
    ...
    if (embeddings[0]?.length !== expectedDim) {
        throw new Error(
            `Invalid embedding dimension: expected ${expectedDim}, got ${embeddings[0]?.length}`
        );
    }
    ...
    try {
        ...
        console.log(
            `[VectorService] Inserting ${embeddings.length} embeddings for document ${documentId}`
        );
        ...
        for (let i = 0; i < embeddings.length; i++) {
            ...
            await client.query(
                ...
                [documentId, i, vectorString, orgId]
            );
        }
        ...
        console.log(
            `[VectorService] Successfully inserted ${embeddings.length} embeddings`
        );
    } catch (error) {
        if (client) {
...
export async function searchSimilarChunks(
    params: SearchSimilarChunksParams
    ...
    if (queryEmbedding.length !== expectedDim) {
        throw new Error(
            `Invalid query embedding dimension: expected ${expectedDim}, got ${queryEmbedding.length}`
        );
    }
    ...
    try {
        ...
        console.log(
            `[VectorService] Searching for top ${limit} similar chunks in org=${orgId}`,
            documentIds ? `filtered to ${documentIds.length} documents` : 'all documents'
        );
        ...
        const queryParams: any[] = [orgId, vectorString];
        let paramIndex = 3;
...
export async function deleteDocumentEmbeddings(
    ...
    try {
        ...
        console.log(
            `[VectorService] Deleted ${result.rowCount} embeddings for document ${documentId}`
        );
